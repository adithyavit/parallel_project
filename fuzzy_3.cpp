...
double d0 =0,d1=0,d2=0;
 ...
 d0 = ((fmin(image.at<cv::Vec3b>(i+k, j+l)[0],image.at<cv::Vec3b>(i, j)[0])+1024)/(fmax(image.at<cv::Vec3b>(i+k, j+l)[0],image.at<cv::Vec3b>(i, j)[0])+1024));
 d1 = ((fmin(image.at<cv::Vec3b>(i+k, j+l)[1],image.at<cv::Vec3b>(i, j)[1])+1024)/(fmax(image.at<cv::Vec3b>(i+k, j+l)[1],image.at<cv::Vec3b>(i, j)[1])+1024));
 d2 = ((fmin(image.at<cv::Vec3b>(i+k, j+l)[2],image.at<cv::Vec3b>(i, j)[2])+1024)/(fmax(image.at<cv::Vec3b>(i+k, j+l)[2],image.at<cv::Vec3b>(i, j)[2])+1024));
}
}
                    double ans = (d0*d1*d2)/9;
                    cout<<ans<<endl;
...
//calculate time
    ...
    cout<<"time taken for fuzzy with 3*3 averaging is: "<<diff<<" seconds"<<endl;
//output image as file
    imwrite("/home/adithya/Desktop/parallel_opencv/fuzzy3.jpg",img);
    return 0;
}
